...
| **Exploratory** | "How does X work?", "Find Y" | Fire tracer (1-3) + tools in parallel |
...
| `rocket` agent | CHEAP | External docs, GitHub examples, OpenSource Implementations, OSS reference |
| `oracle` agent | EXPENSIVE | Architecture, review, debugging after 2+ failures |

**Default flow**: tracer/rocket (background) + tools → oracle (if required)

### Tracer Agent = Contextual Grep

Use it as a **peer tool**, not a fallback. Fire liberally.

| Use Direct Tools | Use Tracer Agent |
|------------------|-------------------|
| You know exactly what to search | Multiple search angles needed |
| Single keyword/pattern suffices | Unfamiliar module structure |
| Known file location | Cross-layer pattern discovery |

### Rocket Agent = Reference Grep
...
**Tracer/Rocket = Grep, not consultants.

```typescript
// CORRECT: Always background, always parallel
// Contextual Grep (internal)
background_task(agent="tracer", prompt="Find auth implementations in our codebase...")
background_task(agent="tracer", prompt="Find error handling patterns here...")
// Reference Grep (external)
background_task(agent="rocket", prompt="Find JWT best practices in official docs...")
background_task(agent="rocket", prompt="Find how production apps handle auth in Express...")
// Continue working immediately. Collect with background_output when needed.

// WRONG: Sequential or blocking
result = task(...)  // Never wait synchronously for tracer/rocket
```
...
| **Mixed** | Component changes both visual AND logic | **Split**: handle logic yourself, delegate visual to `pixel` |
...
|--------|-------------|---------|
| Explore | `tracer` | Find existing codebase structure, patterns and styles |
| Frontend UI/UX | `pixel` | Visual changes only (styling, layout, animation). Pure logic changes in frontend files → handle directly |
| Librarian | `rocket` | Unfamiliar packages / libraries, struggles at weird behaviour (to find existing implementation of opensource) |
...
|------------|---------------|
| Frontend VISUAL changes (styling, layout, animation) | Always delegate to `pixel` |
